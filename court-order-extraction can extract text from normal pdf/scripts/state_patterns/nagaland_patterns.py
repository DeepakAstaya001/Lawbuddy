# Regex patterns for NAGALAND courts
...
PATTERNS = {
    ...
    "alternate_case_number": [
        r"CR\s*(\d+\/\d+)",
        r"C\.R\s*Case\s*No\.\s*(\d+\s*of\s*\d{4})",
        r"Civil Revision\s*(?:No\.)?\s*(\d+\/\d{4})",
        r"Criminal Appeal\s*(?:No\.)?\s*(\d+\/\d{4})"
    ],
    
    "order_date": [
        r"Date of Judgment & Order\s*:\s*([\d{1,2}\.\/\-]+[\d{1,2}\.\/\-]+\d{4})",
